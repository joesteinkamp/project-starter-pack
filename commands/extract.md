---
description: Reverse-engineer PRODUCT.md, DESIGN.md, CODE.md, AGENTS.md from an existing codebase, fill missing briefs, and surface fork points to iterate on
---
...
1. **Scan** — walk the repo for evidence per brief: existing `PRODUCT.md`, `DESIGN.md`, `CODE.md`, `AGENTS.md` / `AGENTS.md`, `CLAUDE.md`, plus manifests (`package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`), framework configs, CSS / Tailwind / token files, component directories, README, and any `.cursor/`, `.github/copilot-instructions.md`, etc.
...
3. **Extract per brief** — for every non-complete brief, derive what's inferable from the codebase. Order by inferability: `CODE.md` first (most code-grounded), then `DESIGN.md` (tokens + components), then `PRODUCT.md` and `AGENTS.md` (least inferable from code; lean on README and existing docs).
...
7. **Re-orchestrate** — invoke the `orchestrator` skill so `AGENTS.md` and `CLAUDE.md` regenerate against the now-complete set.
