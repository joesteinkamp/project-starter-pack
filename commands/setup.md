...
1. **Intro** — In one short paragraph, tell the user what's about to happen: three briefs (Product, Design, Technical), then synthesis into `AGENT.md` and the selected harness files. Ask whether to proceed.
   - **Brownfield check:** if the repo already contains application code (a `package.json`, components, styles), offer to run `/starter:extract` first to pre-fill the briefs from the existing code, then continue the questionnaires from those drafts instead of from blank.
...
5. **Orchestrate** — invoke the `orchestrator` skill. Synthesize `AGENT.md` and the selected harness files.
...
End with a final summary listing every file written and a "next steps" line: test the AI's output by asking it to build something small in the project — it should now read `AGENT.md` (and its harness file) first and respect the briefs.
