...
`AGENT.md` is always written. The harness-specific files — `CLAUDE.md`, and optionally `GEMINI.md`
or a Cursor file — are emitted only for the harnesses you pick at orchestration time (see
...
`project-starter-pack` produces the **project / codebase layer** — `AGENT.md`, `CLAUDE.md`, and
the briefs — which describes *this repository*: its product, its design system, its stack and
...
what's true of the repo. The generated `CLAUDE.md` and `AGENT.md` carry a short "Layering" note
saying exactly this, so any agent reading them knows where each kind of rule lives.
...
`DESIGN.json` token companion, and the generated `AGENT.md` + `CLAUDE.md` — for a fictional
...
system (raw hex where OKLCH applies, glassmorphism, animating layout properties). They are not
installed by default and never block. Run `hooks/install-hooks.sh` to enable them; see
