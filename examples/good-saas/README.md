...
A complete, internally consistent set of briefs for a fictional B2B engineering-retro tool called Bramble. Use this as a reference when running `/starter:setup` — it shows what filled-out briefs look like and how the orchestrator turns them into `AGENT.md` and `CLAUDE.md`.
...
- `AGENT.md` — synthesized universal agents.md
- `CLAUDE.md` — thin import of `AGENT.md` + Claude-Code-specific notes
...
- **CLAUDE.md notes all cite specific files or commands** — `app/lib/retro/service.ts`, `pnpm db:migrate:prod`, the writing-surface route. None of them are generic Claude advice; that lives in `AGENT.md`.

## How this was assembled

The three source briefs (`PRODUCT.md`, `DESIGN.md`, `CODE.md`) were authored first by walking the questionnaires under `/questionnaires/`. The orchestrator then synthesized `AGENT.md` and `CLAUDE.md` from the three. Running `/starter:validate` against this set returns zero cross-brief contradictions and zero anti-pattern hits — it's a regression fixture for the validator skill.
