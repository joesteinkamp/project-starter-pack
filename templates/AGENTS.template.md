# Agent Instructions

> This file is the universal source of truth for any AI coding agent working in this repository. It follows the [agents.md](https://agents.md) convention, so Codex, Cursor, Antigravity, Copilot, and anything else that honors the spec read it automatically; Claude Code reaches it through `CLAUDE.md`. It is generated by `project-starter-pack` from `PRODUCT.md`, `DESIGN.md`, and `CODE.md`. Edit those files and re-run the pack's `orchestrator` flow rather than hand-editing this one.

> **Layering:** {{LAYERING_NOTE}}

## Project Summary

{{PROJECT_SUMMARY}}

## Workflow Expectations

Before any non-trivial change, read the three briefs at the project root:

1. **`PRODUCT.md`** — who this is for, why it exists, brand stance.
2. **`DESIGN.md`** — UX foundation (users, IA, flows, success metrics) **and** UI system (color, type, spacing, motion, components).
3. **`CODE.md`** — stack, architecture, conventions, performance, security.

For any user-facing words — labels, errors, empty states, docs, marketing — also read **`WRITING.md`** (derived from the briefs; regenerated by the `orchestrator` flow).

If a request is ambiguous, ask the user before assuming. Do not invent product context, design tokens, or stack decisions that contradict these briefs.

## Product context

{{PRODUCT_CONTEXT}}

## UX laws

<!-- Synthesized from DESIGN.md UX foundation. -->

{{UX_LAWS}}

## Design laws

<!-- Synthesized from DESIGN.md UI system. -->

{{DESIGN_LAWS}}

## Accessibility

<!-- Synthesized from PRODUCT.md's accessibility commitment plus DESIGN.md's
UX-specific additions. This is the file every agent reads — the commitment
must be visible here, not only in the briefs. -->

{{ACCESSIBILITY_LAWS}}

## Code conventions

<!-- Synthesized from CODE.md. -->

{{CODE_CONVENTIONS}}

## Anti-patterns (always reject)

The following are non-negotiable bans. If the user asks for them anyway, push back once with a concrete alternative; if they insist, comply but flag the trade-off.

### Product

{{PRODUCT_ANTI_PATTERNS}}

### UX

{{UX_ANTI_PATTERNS}}

### Design

{{DESIGN_ANTI_PATTERNS}}

### Writing

{{WRITING_ANTI_PATTERNS}}

### Code

{{CODE_ANTI_PATTERNS}}

## AI Slop self-check

Before showing any UI to the user, run this check:

- [ ] Does this look like a stranger could tell an AI made it? If yes, redesign.
- [ ] Are the color choices a category reflex (purple-for-AI, neon-for-crypto, blue-for-banking)? If yes, reconsider.
- [ ] Is every element earning its place, or is there ornament-for-ornament's-sake?
- [ ] Did I default to a hero-metric layout, identical-card grid, or modal-first thinking? If yes, see `DESIGN.md` for the chosen pattern.
- [ ] Does the copy hedge ("might", "could", "consider") instead of taking a stance?
- [ ] Would a reader who has skimmed a thousand AI-written pages flag this copy ("delve", "It's not X. It's Y.", em-dash clusters)? See `WRITING.md`.

## When to ask the user

- The request contradicts one of the briefs.
- The request requires a decision the briefs don't cover (new section, new flow, new dependency).
- The request would cross a performance / security / accessibility line in `CODE.md` or `DESIGN.md`.
- You are about to take a destructive or hard-to-reverse action.

## Regenerating these instructions

<!-- Filled by the orchestrator with the pack's location on disk and how to
start each flow in this repo's tools — so an agent in a harness without slash
commands (Antigravity, Codex) can still run the pack by name. -->

{{REGENERATION_NOTE}}
