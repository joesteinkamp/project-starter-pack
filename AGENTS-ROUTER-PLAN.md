...
## Who writes AGENTS.md (and the harness files)

- **`setup`** — step 5 ("Orchestrate") is replaced by a lightweight **"Wire up"** step: fill
  the two slots, write `AGENTS.md` + thin `CLAUDE.md`, print the summary. No harness picker:
  `AGENTS.md` covers Codex / Cursor / Antigravity natively and `CLAUDE.md` is 3 lines of
  import — **always write both**, no question asked.
- **Every brief flow** — on Done, if `AGENTS.md` is missing, write it from the template
  (`{{PROJECT_SUMMARY}}` from `PRODUCT.md` if present, else a `[TODO — run product-brief]`
  marker). Idempotent and cheap; covers users who run a single brief without `setup`.
...
each template** with its harness-appropriate `<global layer>` pointer pre-substituted
(`AGENTS.template.md`: "the operator's own global instructions"; `CLAUDE.template.md`:
"your user/global layer (`~/.claude/CLAUDE.md`)"). The `{{LAYERING_NOTE}}` slot disappears.
test.sh §6's "the project layer wins" render check keeps guarding the clause.
...
| `README.md`, `INSTALL.md`, `examples/README.md`, `hooks/README.md` | Command list, "What you get" table (owners change: AGENTS.md → setup/any flow; WRITING.md → design-brief), harness section (no picker), "AGENT.md embeds the bans" → "the briefs embed the bans" |
| `examples/saga-reader/` | Re-render `AGENTS.md` as the router; update `CLAUDE.md` + `WRITING.md` bylines; delete the `.cursor` render |

**test.sh rewiring:**

- **§2** (orchestrator ↔ synthesized templates): retarget — every slot in
  `AGENTS.template.md` + `CLAUDE.template.md` must be named by `skills/setup/SKILL.md`;
...
| Order | Step | Size |
|---|---|---|
| 1 | Router `AGENTS.template.md` + bake layering notes + product bans slot | S |
...
1. **Always write `CLAUDE.md`?** Recommended yes (it's 3 lines; kills the last picker
   question). Alternative: only when the pack is running inside Claude Code.
