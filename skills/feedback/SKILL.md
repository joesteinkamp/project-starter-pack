---
name: feedback
description: Takes corrective feedback on a generated brief (PRODUCT.md, DESIGN.md, CODE.md, AGENT.md, CLAUDE.md) and proposes a focused edit to the affected md file(s). Use when the user says a generated file is wrong, missing detail, contradicts reality, has the wrong tone, or contains a stale anti-pattern. Triggers on phrases like "fix PRODUCT.md", "this brief is wrong", "give feedback on DESIGN.md", "AGENT.md is missing X", "the generated CLAUDE.md got Y wrong".
---
...
1. Locate the plugin root. The templates are at `templates/{PRODUCT,DESIGN,CODE,AGENT,CLAUDE}.template.md` and the guardrails are at `guardrails/{product,ux,design,code}-anti-patterns.md`. You may need to read them later to ground a proposed edit in the same voice the generator used.
2. Identify which generated briefs exist at the project root: `PRODUCT.md`, `DESIGN.md`, `CODE.md`, `AGENT.md`, `CLAUDE.md`. If none exist, stop and tell the user to run `/starter:setup` first.
...
Choices: `PRODUCT.md`, `DESIGN.md`, `CODE.md`, `AGENT.md`, `CLAUDE.md`, `Multiple / not sure`.
- Allow multi-select.
...
`AGENT.md` and `CLAUDE.md` are **derived** from `PRODUCT.md`, `DESIGN.md`, and `CODE.md`. If the affected file is one of those:
